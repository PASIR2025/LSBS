...
const evcFolderInput = $('#evcFolderInput');
...
const confirmSaveProjectBtn = $('#confirmSaveProjectBtn');
const cancelSaveProjectBtn = $('#cancelSaveProjectBtn');

const LIBRARY_DB_NAME = 'pasir_component_library';
const LIBRARY_DB_VERSION = 1;
const LIBRARY_STORE = 'components';
const SCALE_SETTINGS_KEY = 'pasir_editor_scale_settings';
const BOARD_SETTINGS_KEY = 'pasir_editor_board_settings';
...
const state = {
  ...
  library: [],
  libraryMap: new Map(),
  pendingLibraryTypeId: null,
  pendingWaypoints: [],
  undoStack: [],
  redoStack: [],
  maxHistory: 50,
  projectDirty: false,
  lastProjectName: 'proyecto',
  wireDrawFrame: null,
  viewportFrame: null,
...
function selectComponent(el, additive = false) {
  ...
  if (additive && state.selectedComponents.has(el.dataset.id)) {
    ...
    if (getTotalSelectionCount() > 1 || state.selectedWires.size) {
      syncSelectionState('multi', 'multi');
      showMultiInspector();
    } else if (!state.selectedComponents.size) {
      syncSelectionState(null, null);
      renderInspectorEmpty();
    } else {
  ...
  if (getTotalSelectionCount() > 1) showMultiInspector();
  else showComponentInspector(el);
  drawWires();
}


function getTotalSelectionCount() {
  return state.selectedComponents.size + state.selectedWires.size + (state.selectedWireId && !state.selectedWires.has(state.selectedWireId) ? 1 : 0);
}

function setMultiSelect(enabled) {
  state.multiSelectMode = !!enabled;
  multiSelectBtn?.classList.toggle('active', state.multiSelectMode);
  setStatus(state.multiSelectMode ? 'Selección múltiple activada' : 'Selección múltiple desactivada', 'success');
}

function getSelectedWireIds() {
  const ids = new Set(state.selectedWires);
  if (state.selectedWireId) ids.add(state.selectedWireId);
  return [...ids].filter(id => state.wires.some(w => w.id === id));
}

function selectWire(wireId, additive = false) {
  if (!additive) {
    $$('.component').forEach(component => component.classList.remove('selected'));
    state.selectedComponents.clear();
    state.selectedWires.clear();
  }
  if (!state.wires.some(w => w.id === wireId)) return;
  if (additive) {
    if (state.selectedWires.has(wireId)) {
      state.selectedWires.delete(wireId);
      if (state.selectedWireId === wireId) state.selectedWireId = null;
    } else {
      state.selectedWires.add(wireId);
  ...
  if (getTotalSelectionCount() > 1) {
    ...
    syncSelectionState('wire', state.selectedWireId);
    showWireInspector(state.selectedWireId);
  } else {
    syncSelectionState(null, null);
    renderInspectorEmpty();
  }
...
function setBulkWireLabels(visible) {
  const ids = getSelectedWireIds();
  if (!ids.length) return setStatus('No hay cables seleccionados', 'warning');
  recordHistory();
  state.wires.forEach((wire, index) => {
    if (ids.includes(wire.id)) {
      wire.showLabel = !!visible;
      if (!wire.label) wire.label = 'W' + String(index + 1).padStart(2, '0');
    }
  });
  state.showWireLabels = true;
  drawWires();
  showMultiInspector();
  setStatus(visible ? 'Etiquetas de cables visibles' : 'Etiquetas de cables ocultas', 'success');
}

function setBulkDeviceText(mode) {
  const comps = getSelectedComponents();
  if (!comps.length) return setStatus('No hay dispositivos seleccionados', 'warning');
  recordHistory();
  comps.forEach(el => {
    el.dataset.textMode = mode;
    el.dataset.showName = mode === 'none' ? 'false' : 'true';
    updateComponentVisualMeta(el);
  });
  showMultiInspector();
  setStatus('Visibilidad de textos aplicada', 'success');
}

function groupSelectedItems() {
  const comps = getSelectedComponents();
  const wires = getSelectedWireIds();
  if (comps.length + wires.length < 2) return setStatus('Selecciona dos o más elementos para unir', 'warning');
  recordHistory();
  state.groupId = (Number(state.groupId) || 0) + 1;
  const groupId = 'grp_' + state.groupId;
  comps.forEach(el => { el.dataset.groupId = groupId; el.classList.add('grouped'); });
  state.wires.forEach(wire => { if (wires.includes(wire.id)) wire.groupId = groupId; });
  drawWires();
  showMultiInspector();
  setStatus('Elementos unidos/bloqueados como grupo', 'success');
}
...
function ungroupSelectedItems() {
  ...
  recordHistory();
  comps.forEach(el => { delete el.dataset.groupId; el.classList.remove('grouped'); });
  state.wires.forEach(wire => { if (wires.includes(wire.id)) delete wire.groupId; });
  drawWires();
  showMultiInspector();
  setStatus('Grupo liberado', 'success');
}

function showMultiInspector() {
  const compCount = state.selectedComponents.size;
  const wireCount = getSelectedWireIds().length;
  inspectorContent.innerHTML = `
    <div class="inspector-scroll">
      <div class="inspector-title">Selección múltiple</div>
      <div class="inspector-meta">${compCount} dispositivo(s) · ${wireCount} cable(s)</div>
      <details class="inspector-section" open>
        <summary>Acciones rápidas</summary>
        <div class="inspector-section-body">
          <button id="bulkGroupBtn" type="button">Unir / bloquear grupo</button>
          <button id="bulkUngroupBtn" type="button">Desunir grupo</button>
          <button id="bulkDeleteBtn" type="button" class="danger">Eliminar seleccionados</button>
        </div>
      </details>
      <details class="inspector-section" open>
        <summary>Cables seleccionados</summary>
        <div class="inspector-section-body">
          <button id="bulkShowWireLabelsBtn" type="button">Mostrar etiquetas de cable</button>
          <button id="bulkHideWireLabelsBtn" type="button">Ocultar etiquetas de cable</button>
        </div>
      </details>
      <details class="inspector-section" open>
        <summary>Dispositivos seleccionados</summary>
        <div class="inspector-section-body">
          <button id="bulkShowDeviceNameBtn" type="button">Mostrar nombres</button>
          <button id="bulkShowDeviceRefBtn" type="button">Mostrar etiquetas</button>
          <button id="bulkShowDeviceBothBtn" type="button">Nombre + etiqueta</button>
          <button id="bulkHideDeviceTextBtn" type="button">Ocultar textos</button>
        </div>
      </details>
    </div>
    <div class="inspector-actions sticky-actions"><button id="closeInspectorBtn" type="button" class="inspector-secondary-button">Cerrar panel</button></div>`;
  $('#bulkGroupBtn', inspectorContent)?.addEventListener('click', groupSelectedItems);
  $('#bulkUngroupBtn', inspectorContent)?.addEventListener('click', ungroupSelectedItems);
  $('#bulkDeleteBtn', inspectorContent)?.addEventListener('click', deleteSelected);
  $('#bulkShowWireLabelsBtn', inspectorContent)?.addEventListener('click', () => setBulkWireLabels(true));
  $('#bulkHideWireLabelsBtn', inspectorContent)?.addEventListener('click', () => setBulkWireLabels(false));
  $('#bulkShowDeviceNameBtn', inspectorContent)?.addEventListener('click', () => setBulkDeviceText('name'));
  $('#bulkShowDeviceRefBtn', inspectorContent)?.addEventListener('click', () => setBulkDeviceText('ref'));
  $('#bulkShowDeviceBothBtn', inspectorContent)?.addEventListener('click', () => setBulkDeviceText('both'));
  $('#bulkHideDeviceTextBtn', inspectorContent)?.addEventListener('click', () => setBulkDeviceText('none'));
  $('#closeInspectorBtn', inspectorContent)?.addEventListener('click', () => inspector?.classList.add('collapsed'));
  keepInspectorClosedOnMobile();
}
...
function autoNumberDeviceRefs() {
  ...
  getLayerSortedComponents().forEach(el => {
    ...
    if (!el.dataset.textMode || el.dataset.textMode === 'none') {
      el.dataset.textMode = 'ref';
      el.dataset.showName = 'true';
    }
    updateComponentVisualMeta(el);
  });
  drawWires();
...
function startComponentLabelDrag(event, el, tagType) {
  ...
  selectComponent(el, event.shiftKey || state.multiSelectMode);
...
function getComponentTextMode(el) {
  ...
  return (el.dataset.showName || 'false') === 'true' ? 'both' : 'none';
}
...
function keepInspectorClosedOnMobile() {
  if (isMobileViewport()) {
...
function showWireInspector(id) {
  ...
  $('#applyWireProps', inspectorContent).addEventListener('click', () => {
    ...
    // Si el usuario marca mostrar etiquetas desde propiedades, activar tambien
    // la visibilidad global. Antes el cable quedaba marcado internamente,
    // pero no se veia porque state.showWireLabels seguia en false.
    if (wire.showLabel || wire.showEndpointLabels) {
      state.showWireLabels = true;
      if (toggleWireLabelsBtn) {
        toggleWireLabelsBtn.classList.add('active');
        toggleWireLabelsBtn.textContent = 'Etiquetas ON';
      }
    }
...
function buildSmoothCadPath(points){
  ...
  for(let i=1;i<points.length;i++){
    ...
    // v37: no forzar radio minimo grande. Cuando dos nodos estan cerca,
    // mantener la esquina definida para que el cable no parezca unirse o cerrarse.
    const geometryRadius = Math.min(len1 / 2, len2 / 2, 10);
    const r = geometryRadius < 3 ? 0 : geometryRadius;
    if (r <= 0) {
      d += ` L ${curr.x} ${curr.y}`;
      continue;
    }
...
// v34: separa visualmente cables que comparten el mismo borne para que no queden uno encima de otro.
// La conexión lógica sigue siendo el mismo borne; solo desplazamos levemente la salida/entrada visual.
function getEndpointRefKey(ref) {
  return ref ? `${ref.componentId}::${ref.port}` : '';
}

function getEndpointWireOffset(wire, side, direction) {
  const ref = side === 'from' ? wire?.from : wire?.to;
  const key = getEndpointRefKey(ref);
  if (!key) return { x: 0, y: 0 };

  const peers = state.wires
    .filter(other => getEndpointRefKey(other.from) === key || getEndpointRefKey(other.to) === key)
    .sort((a, b) => String(a.id).localeCompare(String(b.id)));

  if (peers.length <= 1) return { x: 0, y: 0 };

  const index = peers.findIndex(other => other.id === wire.id);
  if (index < 0) return { x: 0, y: 0 };

  const maxStroke = Math.max(...peers.map(other => getWireStroke(other)), getWireStroke(wire));
  // Separacion visual equilibrada para varios cables en un mismo borne:
  // suficientemente juntos para verse reales, pero sin montarse uno encima del otro.
  const spacing = Math.max(5, maxStroke * 1.05);
  const amount = (index - (peers.length - 1) / 2) * spacing;

  // Offset perpendicular al lado del borne.
  if (direction === 'top' || direction === 'bottom') return { x: amount, y: 0 };
  if (direction === 'left' || direction === 'right') return { x: 0, y: amount };
  return { x: amount, y: 0 };
}
...
function getWirePoints(wire) {
  ...
  const startBase = { x: startGeometry.x, y: startGeometry.y };
  const endBase = { x: endGeometry.x, y: endGeometry.y };
  const startOffset = getEndpointWireOffset(wire, 'from', startGeometry.direction);
  const endOffset = getEndpointWireOffset(wire, 'to', endGeometry.direction);
  const start = addOffset(startBase, startOffset);
  const end = addOffset(endBase, endOffset);
...
function captureState() {
  return {
    ...
    counters: { componentId: state.componentId, wireId: state.wireId, maxLayer: state.maxLayer, groupId: state.groupId }
  };
}

function updateHistoryButtons() {
  undoBtn && (undoBtn.disabled = state.undoStack.length === 0);
  redoBtn && (redoBtn.disabled = state.redoStack.length === 0);
}
...
function restoreState(snapshot) {
  ...
  state.groupId = snapshot.counters?.groupId || state.groupId || 0;
  ...
  (snapshot.wires || []).forEach(wire => state.wires.push({
    ...
    label: wire.label || '',
    showLabel: wire.showLabel === true,
    showEndpointLabels: wire.showEndpointLabels === true,
...
async function handleFiles(event) {
  const files = [...event.target.files].filter(file => file.name.toLowerCase().endsWith('.evc'));
  if (!files.length) {
    setStatus('No se encontraron archivos .evc', 'warning');
    event.target.value = '';
    return;
  }
  for (const file of files) {
    await loadEvcFile(file, false, getLibraryCategoryFromFile(file));
  }
  renderLibrary();
  setStatus(`Biblioteca cargada: ${files.length} archivo(s) EVC`, 'success');
  event.target.value = '';
}

async function loadEvcFile(file, rerender = true, forcedCategory = null) {
  try {
    ...
    const component = {
      ...
      category: forcedCategory || json.category || 'General',
...
function getLibraryCategoryFromFile(file) {
  const rel = file.webkitRelativePath || file.relativePath || '';
  if (!rel || !rel.includes('/')) return null;
  const parts = rel.split('/').filter(Boolean);
  parts.pop();
  if (!parts.length) return null;
  // Si se eligió una carpeta raíz, no la usamos como categoría única;
  // usamos las subcarpetas internas para estructurar la biblioteca.
  return parts.length > 1 ? parts.slice(1).join(' / ') : parts[0];
}

function selectLibraryComponentForPlacement(typeId) {
  const component = state.libraryMap.get(typeId);
  if (!component) {
    setStatus('No se encontró el componente en la biblioteca', 'error');
    return;
  }
  state.pendingLibraryTypeId = typeId;
  $$('.library-item').forEach(item => item.classList.toggle('selected-for-placement', item.dataset.typeId === typeId));
  closeLibraryDrawer();
  setMode('select');
  setStatus(`Toca el tablero para insertar: ${component.name || typeId}`, 'success');
}

function insertPendingLibraryComponentAt(event) {
  if (!state.pendingLibraryTypeId) return false;
  const component = state.libraryMap.get(state.pendingLibraryTypeId);
  if (!component) {
    state.pendingLibraryTypeId = null;
    return false;
  }
  recordHistory();
  ...
  if (created) {
    state.pendingLibraryTypeId = null;
    $$('.library-item').forEach(item => item.classList.remove('selected-for-placement'));
    selectComponent(created);
    drawWires();
    setStatus('Componente insertado en el tablero', 'success');
    return true;
  }
  setStatus('No se pudo insertar el componente', 'error');
  return false;
...
function renderLibrary() {
  ...
  [...groups.entries()].sort((a, b) => a[0].localeCompare(b[0], 'es')).forEach(([category, components]) => {
    const group = document.createElement('details');
    group.className = 'library-category';
    group.open = true;
    group.innerHTML = `<summary>${category} <span>${components.length}</span></summary>`;

    const grid = document.createElement('div');
    grid.className = 'library-category-grid';

    components.sort((a, b) => (a.name || '').localeCompare(b.name || '', 'es')).forEach(component => {
      const div = document.createElement('div');
      div.className = 'library-item';
      div.draggable = true;
      div.dataset.typeId = component.id;
      div.title = 'Toca este elemento y luego toca el tablero para insertarlo';
      div.innerHTML = `
        <img src="${component.image}" alt="${component.name}">
        <strong>${component.name}</strong>
        <small>${component.terminals.length} bornes</small>
      `;
      div.addEventListener('click', event => {
        event.preventDefault();
        event.stopPropagation();
        selectLibraryComponentForPlacement(component.id);
      });
      // v37 móvil: al desplazar la biblioteca no debe seleccionarse accidentalmente.
      div.addEventListener('touchstart', event => {
        const t = event.touches[0];
        div.dataset.touchStartX = String(t.clientX);
        div.dataset.touchStartY = String(t.clientY);
        div.dataset.touchMoved = 'false';
      }, { passive: true });
      div.addEventListener('touchmove', event => {
        const t = event.touches[0];
        const sx = Number(div.dataset.touchStartX || t.clientX);
        const sy = Number(div.dataset.touchStartY || t.clientY);
        if (Math.hypot(t.clientX - sx, t.clientY - sy) > 8) div.dataset.touchMoved = 'true';
      }, { passive: true });
      div.addEventListener('touchend', event => {
        if (div.dataset.touchMoved === 'true') return;
        event.preventDefault();
        event.stopPropagation();
        selectLibraryComponentForPlacement(component.id);
      }, { passive: false });
...
function addComponent(def, options = {}) {
  ...
  el.dataset.textMode = options.textMode ?? ((options.showName ?? 'false') === 'true' ? 'both' : 'none');
  el.dataset.showName = el.dataset.textMode === 'none' ? 'false' : 'true';
  if (options.groupId) { el.dataset.groupId = options.groupId; el.classList.add('grouped'); }
  ...
  // Soporte movil: permite mover componentes con un dedo sin activar el pan del tablero.
  el.addEventListener('touchstart', event => {
    if (event.touches.length !== 1) return;
    if (state.mode !== 'select' && !state.multiSelectMode) return;
    if (event.target.closest('.port') || event.target.closest('.resize-handle') || event.target.closest('.component-name-tag') || event.target.closest('.component-ref-tag')) return;
    event.preventDefault();
    event.stopPropagation();
    const touch = event.touches[0];
    selectComponent(el, state.multiSelectMode);
    if (el.dataset.locked === 'true') {
      setStatus('Componente bloqueado', 'warning');
      return;
    }
    startComponentDrag({ clientX: touch.clientX, clientY: touch.clientY, button: 0, target: event.target, shiftKey: false }, el);
  }, { passive: false });
...
function startComponentDrag(event, el) {
  ...
  const addTarget = target => { if (target) targets.set(target.dataset.id, target); };
  const selected = getSelectedComponents();
  if (selected.length && selected.some(item => item === el)) selected.forEach(addTarget);
  else addTarget(el);
  const groups = new Set([...targets.values()].map(item => item.dataset.groupId).filter(Boolean));
  groups.forEach(groupId => $$('.component').forEach(item => { if (item.dataset.groupId === groupId) addTarget(item); }));
  const wireOrigins = state.wires
    .filter(wire => wire.groupId && groups.has(wire.groupId))
    .map(wire => ({ wire, waypoints: (wire.waypoints || []).map(point => ({ x: point.x, y: point.y })) }));
  state.dragContext = {
    el,
    startX: event.clientX,
    startY: event.clientY,
    origins: [...targets.values()].map(target => ({
      el: target,
...
function trimPolylineEndpoint(points, distance, fromStart = true) {
  if (!Array.isArray(points) || points.length < 2 || !distance || distance <= 0) return points ? points.map(p => ({ x: p.x, y: p.y })) : points;
  const result = points.map(p => ({ x: p.x, y: p.y }));
  if (!fromStart) result.reverse();

  let remaining = distance;
  while (result.length >= 2 && remaining > 0) {
    const a = result[0];
    const b = result[1];
...
function getVisibleWirePoints(points, wire) {
  if (!points || points.length < 2) return points;
  const terminalType = normalizeWireTerminal(wire?.terminalType || 'ferrule');
  if (terminalType === 'none') return points;
  const metrics = getTerminalMetrics(wire);
  // Recorta el trazo visible para que el cable no sobresalga por debajo de la puntera/terminal.
  const trim = Math.max(4, Math.min(metrics.ferruleLength + 1.5, 22));
  let visible = trimPolylineEndpoint(points, trim, true);
  visible = trimPolylineEndpoint(visible, trim, false);
...
function drawWires() {
  ...
  state.wires.forEach((wire, wireIndex) => {
    ...
    const visiblePoints = getVisibleWirePoints(points, wire);
    const visiblePathData = buildSmoothCadPath(visiblePoints);
    ...
    // v37: el cable se puede seleccionar desde el modo Mover, sin entrar primero a Editar.
    // El trazo de captura es transparente y ancho para que sea comodo en PC y movil.
    hitPath.style.pointerEvents = 'stroke';
    hitPath.style.cursor = state.mode === 'delete-wire' ? 'not-allowed' : 'pointer';
    ...
    hitPath.addEventListener('click', event => {
      ...
      const additiveWireSelect = event.shiftKey || state.multiSelectMode;
      if (additiveWireSelect) {
        selectWire(wire.id, true);
        setStatus('Cable seleccionado en selección múltiple', 'success');
        return;
      }

      // v37: seleccionar el cable desde Mover y mostrar sus nodos inmediatamente.
      // No cambiamos de herramienta; el usuario puede mover nodos sin tocar "Editar".
      selectWire(wire.id, false);
      ...
      if (matching) matching.checked = true;
      drawWires();
      setStatus('Cable seleccionado', 'success');
    });
    ...
    // v37: clic derecho sobre un cable seleccionado crea un nodo justo en ese punto.
    ...
    // v37 móvil: mantener presionado un cable seleccionado crea un nodo en ese punto.
    hitPath.addEventListener('touchstart', event => {
      if (event.touches.length !== 1) return;
      const touch = event.touches[0];
      const startX = touch.clientX;
      const startY = touch.clientY;
      state.lastWireTouchX = startX;
      state.lastWireTouchY = startY;
      clearTimeout(state.wireLongPressTimer);
      state.wireLongPressTimer = setTimeout(() => {
        selectWire(wire.id, false);
        const pt = clientToWorld(startX, startY);
        createNodeOnSelectedWire(pt);
        state.suppressWorkspaceClick = true;
        setTimeout(() => { state.suppressWorkspaceClick = false; }, 120);
      }, 520);
    }, { passive: true });
    hitPath.addEventListener('touchmove', event => {
      if (!event.touches.length) return;
      const touch = event.touches[0];
      if (Math.abs(touch.clientX - (state.lastWireTouchX || touch.clientX)) > 10 || Math.abs(touch.clientY - (state.lastWireTouchY || touch.clientY)) > 10) {
        clearTimeout(state.wireLongPressTimer);
      }
    }, { passive: true });
    hitPath.addEventListener('touchend', () => clearTimeout(state.wireLongPressTimer), { passive: true });

    fragment.appendChild(hitPath);

    const path = document.createElementNS('http://www.w3.org/2000/svg', 'path');
    path.setAttribute('d', visiblePathData);
    path.setAttribute('stroke', wireStrokeColor(wire.color));
    path.setAttribute('stroke-width', getWireStroke(wire));
    path.setAttribute('fill', 'none');
    path.setAttribute('stroke-linecap', 'round');
    path.setAttribute('stroke-linejoin', 'round');
    path.setAttribute('class', 'wire-path' + ((state.selectedWireId === wire.id || state.selectedWires.has(wire.id)) ? ' selected-wire' : ''));
    path.style.pointerEvents = 'none';
    fragment.appendChild(path);
    if (wire.color === 'green-yellow') {
      const ypath = document.createElementNS('http://www.w3.org/2000/svg', 'path');
      ypath.setAttribute('d', visiblePathData);
    ...
    (wire.waypoints || []).forEach((point, index) => {
      ...
      function beginWireNodeDrag(eventLike) {
        selectWire(wire.id, false);
        selectedNodeIndex = index;
        state.draggingNode = {
          wire,
          ...
          pointerId: eventLike.pointerId ?? null
        };
        node.classList.add('selected');
        setStatus(`Nodo ${index + 1} listo para mover`, 'success');
      }
      ...
      // Soporte tactil: mover nodos de cable con un dedo en movil/tablet.
      node.addEventListener('touchstart', event => {
        if (event.touches.length !== 1) return;
        event.preventDefault();
        event.stopPropagation();
        const touch = event.touches[0];
        beginWireNodeDrag({ clientX: touch.clientX, clientY: touch.clientY, pointerId: null });
      }, { passive: false });
...
function deleteSelected() {
  ...
  const selectedWireIds = getSelectedWireIds();
  if (selectedComponents.length || selectedWireIds.length) {
    recordHistory();
    const componentIds = new Set(selectedComponents.map(component => component.dataset.id));
    ...
    for (let index = state.wires.length - 1; index >= 0; index -= 1) {
      ...
      if (selectedWireIds.includes(wire.id) || componentIds.has(wire.from.componentId) || componentIds.has(wire.to.componentId)) state.wires.splice(index, 1);
    }
    state.selectedComponents.clear();
    state.selectedWires.clear();
    state.selectedWireId = null;
    ...
    renderInspectorEmpty();
    drawWires();
    setStatus('Selección eliminada', 'success');
    return;
  }
  setStatus('No hay selección para eliminar', 'warning');
}
...
function getProjectTitle() {
  ...
  return `Pasir Editor - Tablero ${state.boardWidthMm} x ${state.boardHeightMm} mm - ${stamp}`;
}
...
function exportSvgFile() {
  downloadTextFile('tablero-pasir.svg', buildBoardSvg(), 'image/svg+xml;charset=utf-8');
...
function exportPdfPrint() {
  ...
  win.document.write(`<!DOCTYPE html><html lang="es"><head><meta charset="UTF-8"><title>${xmlEscape(getProjectTitle())}</title><style>
    ...
    <section>
      <header><div><h1>Lista de materiales / BOM</h1><div class="meta">Generado desde Pasir Editor</div></div></header>
      ${bom}
    </section>
...
function sanitizeProjectFileName(name) {
  const cleaned = String(name || 'proyecto')
    .trim()
    .replace(/\.pasir$/i, '')
    .replace(/[\\/:*?"<>|]+/g, '-')
    .replace(/\s+/g, ' ')
...
function showSaveProjectModal() {
  if (!saveProjectModal || !projectNameInput) {
    saveProject(state.lastProjectName || 'proyecto');
    return;
  }
  projectNameInput.value = sanitizeProjectFileName(state.lastProjectName || 'proyecto');
  saveProjectModal.hidden = false;
  setTimeout(() => projectNameInput.select(), 0);
}

function hideSaveProjectModal() {
  if (saveProjectModal) saveProjectModal.hidden = true;
}
...
function buildProjectPayload() {
  ...
  const payload = {
    ...
    editor: { snapEnabled: state.snapEnabled, wireRouteMode: state.currentWireRouteMode, wireTerminal: state.currentWireTerminal, terminalColor: state.currentTerminalColor, pxPerMm: state.pxPerMm, showRealUnits: state.showRealUnits, showWireLabels: state.showWireLabels },
...
function saveProject(name = state.lastProjectName || 'proyecto') {
  const fileBaseName = sanitizeProjectFileName(name);
  state.lastProjectName = fileBaseName;
  const payload = buildProjectPayload();
  payload.projectName = fileBaseName;
  payload.brand = 'Pasir Editor';
  payload.fileType = 'pasir-project';

  const blob = new Blob([JSON.stringify(payload, null, 2)], { type: 'application/x-pasir-project;charset=utf-8' });
  const link = document.createElement('a');
  link.href = URL.createObjectURL(blob);
  link.download = `${fileBaseName}.pasir`;
  link.click();
  URL.revokeObjectURL(link.href);
  state.projectDirty = false;
  hideSaveProjectModal();
  setStatus(`Proyecto guardado: ${fileBaseName}.pasir`, 'success');
}

function newProject() {
  const hasDesign = $$('.component').length > 0 || state.wires.length > 0;
  if (hasDesign && !confirm('Se borrará el diseño actual. ¿Deseas crear un proyecto nuevo?')) return;
  recordHistory();
  clearPendingWire();
  canvas.innerHTML = '';
  state.wires.length = 0;
  ...
  state.maxLayer = 0;
  state.groupId = 0;
  state.lastProjectName = 'proyecto';
  syncSelectionState(null, null);
  renderInspectorEmpty();
  requestDrawWires();
  state.projectDirty = false;
  setStatus('Nuevo proyecto creado', 'success');
}
...
async function openProjectFile(event) {
  ...
  try {
    ...
    state.lastProjectName = sanitizeProjectFileName(data.projectName || file.name.replace(/\.(pasir|evd|json)$/i, '') || 'proyecto');
    state.projectDirty = false;
...
async function restoreProject(data) {
  ...
  state.groupId = data.counters?.groupId || state.groupId || 0;
...
multiSelectBtn?.addEventListener('click', () => setMultiSelect(!state.multiSelectMode));
...
evcFolderInput?.addEventListener('change', handleFiles);
...
confirmSaveProjectBtn?.addEventListener('click', () => saveProject(projectNameInput?.value || state.lastProjectName));
cancelSaveProjectBtn?.addEventListener('click', hideSaveProjectModal);
saveProjectModal?.addEventListener('click', event => { if (event.target === saveProjectModal) hideSaveProjectModal(); });
projectNameInput?.addEventListener('keydown', event => {
  if (event.key === 'Enter') saveProject(projectNameInput.value);
  if (event.key === 'Escape') hideSaveProjectModal();
});
...
bindTapAction(libraryBtn, () => openLibraryDrawer());
bindTapAction(closeLibraryBtn, () => closeLibraryDrawer());
bindTapAction(drawerBackdrop, () => closeLibraryDrawer());
...
workspace.addEventListener('mousedown', event => {
  if (state.pendingLibraryTypeId && isWorkspaceBackgroundTarget(event.target)) {
    event.preventDefault();
    insertPendingLibraryComponentAt(event);
    return;
  }
  const blank = !event.target.closest('.inspector') && !event.target.closest('.component') && !event.target.closest('.wire-node') && !event.target.closest('path');
  if (blank && state.mode === 'select' && !state.spacePressed) {
    clearSelection();
  }
...
workspace.addEventListener('mousemove', event => {
  if (state.draggingNode) {
    ...
    document.body.classList.add('performance-dragging');
...
workspace.addEventListener('click', event => {
  ...
  if (state.mode === 'wire' && state.pendingPort) {
    addWaypoint(getWorkspacePoint(event));
    return;
  }
  // Al tocar/clickear un area blanca se cancela cualquier herramienta activa,
  // excepto cuando ya se inició un cable: en ese caso agrega trayectoria.
  if (state.mode !== 'select') {
    clearPendingWire();
    setMode('select');
    clearSelection();
...
document.addEventListener('mousemove', event => {
  ...
  if (state.dragContext) {
    ...
    (state.dragContext.wireOrigins || []).forEach(item => {
      item.wire.waypoints = item.waypoints.map(point => ({ x: point.x + dx, y: point.y + dy }));
    });
    requestDrawWires();
  }
});
...
document.addEventListener('mouseup', () => {
  ...
  if (state.resizeContext) {
    ...
    document.body.classList.remove('performance-dragging');
  }
...
workspace.addEventListener('touchstart', event => {
  ...
  if (isWorkspaceBackgroundTarget(event.target)) {
    event.preventDefault();
    if (state.pendingLibraryTypeId) {
      insertPendingLibraryComponentAt(fakeEvent);
      return;
    }
    if (state.mode === 'wire' && state.pendingPort) {
      addWaypoint(getWorkspacePoint(fakeEvent));
      return;
    }
    clearSelection();
    if (state.mode !== 'select') {
...
document.addEventListener('pointermove', event => {
  ...
  // Evita que el pan o la seleccion capturen el movimiento del nodo.
  event.preventDefault();
  ...
  state.draggingNode.started = true;
  state.draggingNode.wire.waypoints[state.draggingNode.index] = state.snapEnabled ? snapPoint(point) : point;
  requestDrawWires();
}, { passive: false });
...
document.addEventListener('touchmove', event => {
  ...
  if (state.draggingNode && event.touches.length === 1) {
    event.preventDefault();
    const touch = event.touches[0];
  ...
  if (state.dragContext && event.touches.length === 1) {
    event.preventDefault();
    const touch = event.touches[0];
    const dx = (touch.clientX - state.dragContext.startX) / state.zoom;
    const dy = (touch.clientY - state.dragContext.startY) / state.zoom;
    if (!state.dragContext.snapshotSaved && Math.hypot(dx, dy) > 2) {
      recordHistory();
      state.dragContext.snapshotSaved = true;
    }
...
document.addEventListener('touchend', event => {
  ...
  if (state.draggingNode && event.touches.length === 0) {
    if (state.draggingNode.started) {
      drawWires();
      setStatus('Nodo actualizado', 'success');
    }
  ...
  if (state.dragContext && event.touches.length === 0) {
    if (state.dragContext.moved) {
      setStatus('Componente movido', 'success');
      showComponentInspector(state.dragContext.el);
      drawWires();
...
// Registro PWA: permite instalar la app cuando se sirve por http/https.
